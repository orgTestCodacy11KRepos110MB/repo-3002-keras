...
class OptimizersTest(test_util.DTensorBaseTest):
    ...
    def test_embedding_lookup_backward_path(self):
        ...
        # Note that the small batch size is critical here to trigger the error
        # which is related to constant folding/caching in DTensor. Larger
        # size (eg > 20) will not trigger the error.
        batch_size = 16
        seq_length = 10
        vocab_size = 100
        output_size = 8
        ...
        def produce_data():
            inputs = tf.random.uniform(
                maxval=vocab_size, shape=(batch_size, seq_length), dtype=tf.int32)
            ...
            inputs = dtensor.relayout(inputs, dtensor.Layout.batch_sharded(self.mesh, 'X', 2))
            label = dtensor.copy_to_mesh(
                label, layout=dtensor.Layout.replicated(self.mesh, rank=1))
            label = dtensor.relayout(label, dtensor.Layout.batch_sharded(self.mesh, 'X', 1))
            return inputs, label
